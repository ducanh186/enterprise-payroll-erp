    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <input
          ...
          placeholder="Tìm theo mã hoặc tên phụ cấp..."
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
          ...
          <tbody className="divide-y divide-slate-100">
            {query.isLoading ? (
              ...
              filtered.map((item, index) => {
                ...
                const insurable = boolValue(item, ["is_insurable", "insurable", "subject_to_insurance"], false);
                ...
                return (
                  <tr key={`${id}-${index}`} className="transition-colors hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-4 text-sm font-semibold tabular-nums text-slate-900">
                      {defaultAmount ? formatCurrency(defaultAmount) : "—"}
                    </td>
                    <td className="px-4 py-4 text-center">
                      {taxable ? (
                        <Badge tone="warning">Có</Badge>
                      ) : (
                        <Badge tone="neutral">Không</Badge>
                      )}
                    </td>
                    <td className="px-4 py-4 text-center">
                      {insurable ? (
        ...
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/30 px-6 py-4">
          <p className="text-xs text-slate-500">
            ...
            <span className="font-bold text-slate-800">{items.length}</span> phụ cấp
          </p>
        </div>
      </div>

      {/* Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Thêm phụ cấp mới" size="md">
        <form className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700">Mã phụ cấp</label>
              <input type="text" placeholder="Ví dụ: PC_DI_LAI" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700">Tên phụ cấp</label>
              <input type="text" placeholder="Phụ cấp đi lại" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700">Mức mặc định (VNĐ)</label>
              <input type="number" placeholder="500000" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100" />
            </div>
          </div>
          <div className="flex gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
              <span className="text-sm font-medium text-slate-700">Chịu thuế TNCN</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
              <span className="text-sm font-medium text-slate-700">Đóng bảo hiểm</span>
            </label>
          </div>
