...
function leaveStatusBadge(status: string) {
  ...
  if (s.includes("approved") || s.includes("accept") || s.includes("duyệt")) return <Badge tone="success">Đã duyệt</Badge>;
  ...
  if (s.includes("cancelled") || s.includes("cancel") || s.includes("hủy")) return <Badge tone="neutral">Đã hủy</Badge>;
...
export default function LeaveRequestsPage() {
  ...
  const query = useQuery({
    ...
    queryFn: async () =>
      apiGet<unknown>("/attendance/requests", { page: 1, per_page: 30 }),
  });
  ...
  const filtered = useMemo(() => {
    return items.filter((item) => {
      ...
      const type = textValue(item, ["leave_type", "type", "request_type"], "").toLowerCase();
  ...
  return (
    <div className="space-y-8 pb-10">
      <PageHeader
        ...
        description="Quản lý các đơn xin nghỉ phép của nhân viên và trạng thái phê duyệt."
        actions={
          <>
            <button
              type="button"
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Tìm theo nhân viên hoặc loại nghỉ..."
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
          ...
          <tbody className="divide-y divide-slate-100">
            {query.isLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="animate-pulse">
                  ...
                  {Array.from({ length: 6 }).map((__, j) => (
                    <td key={j} className="px-4 py-4"><div className="h-3.5 w-20 rounded bg-slate-200" /></td>
                  ))}
                </tr>
              ))
            ) : filtered.length ? (
              filtered.map((item, index) => {
                ...
                const type = textValue(item, ["leave_type", "type", "request_type"], "—");
                const fromDate = textValue(item, ["from_date", "start_date"], "");
                const toDate = textValue(item, ["to_date", "end_date"], "");
                const days = numberValue(item, ["days", "total_days", "duration"], 0);
                const reason = textValue(item, ["reason", "note", "description"], "—");
                ...
                return (
                  <tr key={`${id}-${index}`} className="transition-colors hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-4">
                      <span className="rounded bg-sky-50 px-2 py-1 text-[10px] font-bold text-sky-700">{type}</span>
                    </td>
                    ...
                    <td className="px-4 py-4 max-w-xs text-sm text-slate-500 truncate">{reason}</td>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/30 px-6 py-4">
          <p className="text-xs text-slate-500">
            ...
            <span className="font-bold text-slate-800">{items.length}</span> đơn
          </p>
        </div>
      </div>

      {/* Create Leave Request Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Tạo đơn xin nghỉ phép" size="md">
        <form className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-semibold text-slate-700">Loại nghỉ phép</label>
          ...
          <div>
            ...
            <textarea rows={3} placeholder="Lý do xin nghỉ..." className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100" />
          </div>
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="button" onClick={() => setShowModal(false)} className="rounded-xl bg-gradient-to-br from-slate-950 to-indigo-700 px-5 py-2.5 text-sm font-bold text-white shadow-lg transition hover:opacity-90 active:scale-95">Gửi đơn</button>
          </div>
        </form>
      </Modal>
    </div>
